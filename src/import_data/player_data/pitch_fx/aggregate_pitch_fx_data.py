        ...
        for ball in range(4):
            for strike in range(3):
                ...
                for pitch_type in set(pitch_fx_db.read('select pitch_type ' + bulk_query + 'and swing_take = '
                    ...
                    pitch_usage[matchup][count][pitch_type[0]] = int(pitch_fx_db.read('select count(*) '
                                                                                      + temp_bulk_query + ';')[0][0])
                    pitch_type_outcomes[matchup][count][pitch_type[0]] = {}
                    pitch_type_swing_rate[matchup][count][pitch_type[0]] = pitch_fx_db.read('select count(*) '
                                                                                            + temp_bulk_query + ' and'
                                                                                            ' swing_take = '
                                                                                            '"swing";')[0][0]
                    strike_percents[matchup][count][pitch_type[0]] = pitch_fx_db.read('select count(*) '
                                                                                      + temp_bulk_query + ' and '
                                                                                      'ball_strike = "strike";')[0][0]

...
def hitter_spray_chart_constructor(year, driver_logger):
    ...
    if year >= 1988:
        ...
        pt_uid_players = set(DB_Connect.read(cursor, 'select PT_uniqueidentifier from player_batting where year = '
                                                     + str(year) + ';'))
        DB_Connect.close(db)
...
def reduce_functionality(year, ent):
    ...
    stats2 = {"Ground_Balls_PA": "",
              ...
              "Line_Drives_HR": ""}
    db, cursor = DB_Connect.grab("baseballData")
    player_id = DB_Connect.read(cursor, "select playerId from player_teams where PT_uniqueidentifier = "
                                        + str(ent[0]) + ";")[0][0]
    if DB_Connect.read(cursor, 'select pa_infield from player_batting where PT_uniqueidentifier = ' + str(ent[0])
...
def write_to_file(year, pt_uid, stat_list1, stat_list2):
    ...
    DB_Connect.write(db, cursor, 'update player_batting set ' + query_string[:-2] + ' where PT_uniqueidentifier = '
...
# hitter_spray_chart_constructor(2018, Logger("C:\\Users\\Anthony Raimondo\\PycharmProjects\\baseball-sync\\logs\\"

...
logging.basicConfig(filename="C:\\Users\\Anthony Raimondo\\PycharmProjects\\baseball-sync\\logs\\import_data\\"
                             "year_data.log", level=logging.DEBUG)
...
def get_year_data(year):
    ...
    logging.info("\tmaking HTTP requests for year data")
    with ThreadPoolExecutor() as executor1:
        for key, value in stat_list.items():
            executor1.submit(load_url, year, key)
    logging.info("\t\tdone making HTTP requests: time = " + str(round(time.time() - download_start, 2)))
    ...
    logging.info("\t\tdone writing to database: time = " + str(round(time.time() - write_start, 2)))
    total_time = round(time.time() - start_time, 2)
    logging.info('\tyear_data download completed: time = ' + str(total_time) + ' seconds\n\n')


def load_url(year, stat_type):
    logging.info("\tdownloading " + stat_type + " data")
...
def assemble_stats(stat_type, stats, page):
    ...
    start_time = time.time()
    logging.info('\tAssembling year ' + stat_type + ' stats')
...
def write_to_db(year, stat_string, stat_type):
    logging.info("\twriting " + stat_type + " stats to database")
    db, cursor = DB_Connect.grab("baseballData")
